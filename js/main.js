...
/* --- Draw modal (Losuj/Start) helpers --- */
function openDrawModal(selectedCompetitors) {
    // Ensure State.state.competitors is set (Handlers.handleStartCompetition did this)
    // Render list from State.state.competitors
    const drawModal = document.getElementById('drawModal');
    if (!drawModal) {
        console.warn('Draw modal not found in DOM.');
        return;
    }
    renderDrawList();
...
function renderDrawList() {
    ...
    comps.forEach((c, idx) => {
        // c may be a name or object - handle both
        let label = c;
        if (typeof c === 'object') label = c.name || c.fullName || c.id || JSON.stringify(c);
...
function shuffleForDraw() {
    // Use existing State.shuffleCompetitors if available; fallback to Fisher-Yates
    if (State.shuffleCompetitors) {
        State.shuffleCompetitors();
    } else {
        const arr = State.state.competitors || [];
...
// Attach modal button listeners once DOM is ready
document.addEventListener('DOMContentLoaded', () => {
    const btnLosuj = document.getElementById('btnLosuj');
    const btnStart = document.getElementById('btnStart');
    const btnCancel = document.getElementById('btnCancel');
    const drawModal = document.getElementById('drawModal');

    if (btnLosuj) btnLosuj.addEventListener('click', () => {
        shuffleForDraw();
    });

    if (btnStart) btnStart.addEventListener('click', () => {
        // Close modal and refresh UI to show competitors in drawn order
        if (drawModal) drawModal.classList.add('hidden');
        // Hide old shuffle button if exists (cleanup)
        const oldShuffle = document.getElementById('');
    ...
    if (btnCancel) btnCancel.addEventListener('click', () => {
        if (drawModal) drawModal.classList.add('hidden');
        // Optionally revert competitors order? We'll keep current order.
    });
});
...
function setupEventListeners() {
    ...
    safeAddListener('startCompetitionBtn','click', () => {
        if (Handlers.handleStartCompetition()) {
            // Open draw modal to allow losowanie kolejności przed startem
            openDrawModal(State.state.competitors);
        }
    });
    ...
    // --- Main Screen Actions ---    safeAddListener('showResultsBtn','click', UI.toggleHistoryPanel);
